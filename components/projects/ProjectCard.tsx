...
export function ProjectCard({ project }: { project: ProjectSummary }) {
  return (
    <Link
      ...
      className="flex flex-col gap-3 rounded-2xl border border-neutral-200 bg-gradient-to-b from-white/0 to-white p-5 shadow-[0_20px_45px_rgba(8,60,32,0.05)] transition hover:-translate-y-1 hover:shadow-[0_35px_75px_rgba(8,60,32,0.1)]"
    >
      <div className="text-xs uppercase tracking-[0.3em] text-neutral-500">{project.status || "Available"}</div>
      ...
      <p className="text-lg font-semibold text-[var(--brand-dark)]">
        {project.price ? `₹${project.price.toLocaleString("en-IN")}` : "Contact for price"}
      </p>
